...
if re.findall(metadata_regex_ord, imgdata):
    for i in re.findall(metadata_regex_ord, imgdata):
        ...
        if match and match.group(0) not in dupes and "bool" not in match.group(0) and "long" not in match.group(0) and not re.search(r"([A-Z]{1,5}[a-z]{1,5}[A-Z]{1,5})|(.*\.|-$)|([A-Za-z]{1,5}[0-9]{1,5}[A-Za-z]{1,5})|([0-9]{1,5}[A-Za-z]{1,5})", match.group(0)):
